...
-- P0 修复（2026-09-07）：对齐“新版提案审批”路由与 schema，消除缺列导致的 500 回滚
-- 公告倒排自动排期：计划发布日(相对 D 日)与所属业务阶段 key
...
-- 公告模板：D 日倒排偏移量（null=不自动排期；负数=D 日前 N 天），为审批通过时自动生成公告的锚点
...
-- 提案：驳回原因（审批驳回时记录，供前端/小程序展示驳回意见）
alter table election_proposals add column if not exists reject_reason text;
-- 提案：审批通过时自动创建的选举封地 id（归档台账据此挂到正确活动；未回填时按 org 最新封地兜底）
