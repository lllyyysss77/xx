...
// ============================================================
// [TAG-INDEX] materials.ts — 报名材料（上传/审核/内推代建）
// [ROUTE-CORE]   POST /admin/materials — 内推代建（创建用户+材料）
// [ROUTE-CORE]   GET  /admin/materials — 材料列表
// [BREAKPOINT]    PATCH /admin/materials/:id/review — 材料审核断点：通过→自动 insert candidates（幂等 on conflict）
// [ROUTE-CORE]   POST /admin/materials/:id/file — 材料附件上传
// [TODO-FIX P2]   内推代建默认密码 123456，创建响应未提示强制修改
// ============================================================
export async function materialsRoutes(app: FastifyInstance) {
  ...
  // 审核：通过 → 自动进候选人池（幂等）
  // [BREAKPOINT] 材料审核断点 — 通过→自动 insert candidates（on conflict do nothing 幂等），驳回→status=rejected；这是材料→候选人池的状态转换点
