...
// ============================================================
// [TAG-INDEX] accounts.ts — 账号/归属地/批量预设
// [AUTH]  POST /admin/accounts — 直建账号（平台超管/子管理/编辑/审核员，scrypt 哈希）
// [AUTH]  POST /admin/accounts/preset — 批量预设候选人（解锁码校验，幂等 on conflict）
// [ROUTE-CORE] POST /admin/organizations — 新建归属地（村/社区，自动建单位）
// [ROUTE-CORE] GET /admin/accounts — 账号列表
// [TODO-FIX P2]   解锁码默认 '123456' 硬编码（应走环境变量）
// ============================================================
export async function accountsRoutes(app: FastifyInstance) {
  ...
  app.post('/admin/organizations', { preHandler: roleGuard('platform_admin') }, async (req, rep) => {
    const b = z.object({
      ...
      name: z.string().min(1),
      orgType: z.enum(['village', 'community']).default('village'),
    }).parse(req.body);

    // slug 未传或非标准时自动由拼音/时间戳衍生稳定 slug
    const finalSlug = (b.slug && /^[a-z0-9-]+$/.test(b.slug))
      ? b.slug
      : `org-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;

    const r = await pool!.query(
      `insert into organizations(slug,name,org_type,status)
       values($1,$2,$3,'active')
       on conflict (slug) do nothing
       returning id, slug, name, org_type as "orgType", status, created_at as "createdAt"`,
      [finalSlug, b.name, b.orgType],
    );
  ...
  app.get('/admin/organizations', { preHandler: roleGuard('platform_admin') }, async () => {
    return (await pool!.query('select id,slug,name,org_type as "orgType",status,created_at from organizations order by created_at')).rows;
  });

  // 账号列表（超管看全部，子管理看本归属地）
  app.get('/admin/accounts', { preHandler: roleGuard('platform_admin', 'sub_admin') }, async (req, rep) => {
    if (!pool) return rep.code(503).send({ error: 'database_not_configured' });
    const query = (req.query || {}) as { orgId?: string; organizationId?: string };
    const orgId = query.orgId || query.organizationId;
  ...
  app.put('/admin/accounts/:id/reset-password', { preHandler: roleGuard('platform_admin', 'sub_admin') }, async (req, rep) => {
    if (!pool) return rep.code(503).send({ error: 'database_not_configured' });
    const b = z.object({ password: z.string().min(6).default('123456') }).default({}).parse(req.body ?? {});
    const { id } = req.params as { id: string };
    if (req.auth!.role === 'sub_admin') {
      const own = (await pool!.query('select 1 from memberships where user_id=$1 and organization_id=$2', [id, req.auth!.organizationId])).rowCount;
      if (!own) return rep.code(403).send({ error: 'organization_mismatch' });
