...
import { z, pool, roleGuard, orgScope } from '../lib';

// ============================================================
// [TAG-INDEX] notifications.ts — 通知与 webhook 订阅管理
// [CORE-FLOW] GET /admin/notifications — 通知列表
// [CORE-FLOW] POST /admin/notifications/:id/read — 标记已读
// [ROUTE-CORE] GET /admin/webhooks — webhook 订阅列表
// [ROUTE-CORE] POST /admin/webhooks — 创建 webhook 订阅（plain/wecom/feishu）
// [ROUTE-CORE] PATCH /admin/webhooks/:id — 编辑/启停订阅
// ============================================================
export async function notificationsRoutes(app: FastifyInstance) {
  // Webhook 能外发政务消息，仅归属地负责人可管理；不能只靠 Web 的 PermGate。
  const webhookManager = roleGuard('platform_admin', 'sub_admin');

  app.get('/admin/webhook-subscriptions', { preHandler: webhookManager }, async (req, rep) => {
  ...
  app.post('/admin/webhook-subscriptions', { preHandler: webhookManager }, async (req, rep) => {
  ...
  app.patch('/admin/webhook-subscriptions/:id', { preHandler: webhookManager }, async (req, rep) => {
  ...
  app.delete('/admin/webhook-subscriptions/:id', { preHandler: webhookManager }, async (req, rep) => {
    const id = z.string().uuid().parse((req.params as { id: string }).id);
  ...
  app.post('/admin/webhook-subscriptions/:id/test', { preHandler: webhookManager }, async (req, rep) => {
    const id = z.string().uuid().parse((req.params as { id: string }).id);
